...
import { useSession } from "next-auth/react"
import { useRouter } from "next/navigation"
import { Plus, Eye, Trash2, Pencil, ChevronRight, ChevronLeft } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function WatchlistSidebar({ data }) {
  ...
  return (
    <div className="flex h-full relative">
      ...
      {/* Sidebar content */}
      <div
        className={`bg-card h-full border-l transition-all duration-300 ease-in-out ${isCollapsed ? "w-0 opacity-0 overflow-hidden" : "w-80 opacity-100"}`}
      >
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold tracking-tight">Portfolios</h2>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="risk" className="text-base">
                Risk Level
              </Label>
              <Select
                value={newPortfolio.riskLevel}
